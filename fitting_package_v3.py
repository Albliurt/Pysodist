...
#     fourier_array += freq*np.exp(temp_fourier_array)
        ...
        for i in range(self.ResidueInfo.num_species):
            ...
            self.unmixed_ft_residue_models.append(res_init)
    ...
    def Convolution(self, ft_spectra,mults,names=None):
        ...
        #conv = np.prod(np.vstack([ft_spectra[names[i]]**mults[i] for i in range(len(mults))]), axis = 0)
        '''print(ft_spectra.values())
        print(mults)'''
        length = self.N//2 + 1#len(ft_spectra[names[0]])
        #conv = np.prod([ft_spectra[names[i]]**mults[i] for i in range(len(mults))], axis = 0)
#        conv = np.prod(np.power([ft_spectra[names[i]] for i in range(len(mults))], mults), axis = 1)
        # print(ft_spectra)
        # for i in range(len(mults)):
        #     print(names[i])
        #     #print(ft_spectra[names[i]])
        # print("ASDFASDFASDFASDFASDFASDFASDFASDFASD")
        # print(pows)
        # print(trunc_spec)
#        starttime = time.time()
        #trunc_spec = [ft_spectra[names[i]]**mults[i] for i in range(len(mults))]
        #trunc_spec = [ft_spectra[name]**mult for name, mult in zip(names[:len(mults)], mults)]
        # print(list(zip(names[:len(mults)], mults)))
        # trunc_spec = list(map(lambda x: np.exp(x[1]*np.log(ft_spectra[x[0]])), list(zip(names[:len(mults)], mults))))

        # trunc_spec = list(map(lambda x: ft_spectra[x[0]]**x[1], list(zip(names[:len(mults)], mults))))
        # conv = reduce(lambda x,y: x*y, trunc_spec)

        #pows = [length*[mult] for mult in mults]
        #pow_spec = np.power(trunc_spec, pows)
#        self.totaltime += time.time() - starttime
        #reduce(lambda )
        # #    conv = np.multiply(conv, np.power(ft_spectra[names[i]],mults[i]))
        # conv = np.prod(pow_spec, axis = 0)
