...
#atomfile = "C:\\Users\\alber\\Desktop\\IsodistTest\\model_files\\atoms.txt"
...
def signal(sequence, charge, vars, mw, amps = None, gw = 0.003, labeling = 'C13', m_off = 0, base = 0, dm = float(options['dm']), left_mass_pad = 2.5, right_mass_pad = 2.5):
    ...
    for i in range(ResidueInfo.num_species):
        res_init = dict()
        for res in ResidueInfo.residue_info:
            res_init[res] = None
        ft_residue_models.append(res_init)
        unmixed_ft_residue_models.append(res_init.copy())

    for residue in ResidueInfo.residue_info:
        for i in range(ResidueInfo.num_species):
            names = ResidueInfo.atom_names
            unmixed_ft_residue_models[i][residue] = Convolution(ft_atom_models, ResidueInfo.residue_info[residue][i], names)
    ...
    ft_species_models = [Convolution(ft_residue_models[j], res_mults, res_syms) for j in range(ResidueInfo.num_species)]
...
for i in range(len(ResidueInfo.atom_names)):
